...
then
	...
	echo "docker run --rm --network host -p 192.168.3.1:18443:18443/tcp -p 0.0.0.0:18080:18080/tcp -p 0.0.0.0:${ADMINP}:${ADMINP} -v "$PWD/controller.crt":/opt/bitnami/envoy/conf/controller.crt -v $PWD/${ENVOY_CFG}:/opt/bitnami/envoy/conf/envoy.yaml -v $PWD/cert.pem:/etc/envoy/cert.pem -v $PWD/key.pem:/etc/envoy/key.pem --name envoy -t bitnami/envoy >& /tmp/docker.envoy.log"
	docker run --rm --network host -p 192.168.3.1:18443:18443/tcp -p 192.168.3.1:18080:18080/tcp -p 0.0.0.0:${ADMINP}:${ADMINP} -v "$PWD/controller.crt":/etc/envoy/controller.crt -v "$PWD/${ENVOY_CFG}":/opt/bitnami/envoy/conf/envoy.yaml -v "$PWD/cert.pem":/etc/envoy/cert.pem -v "$PWD/key.pem":/etc/envoy/key.pem --name envoy -t bitnami/envoy >& /tmp/docker.envoy.log
fi
